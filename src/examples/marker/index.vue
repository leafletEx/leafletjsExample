...
const createMarker = () => {
  ...
  // 创建一个 marker
  marker.value = new Marker([32.0237855, 118.8075675]).addTo(mapObj.value);
...
const createMarkers = () => {
  ...
  const markers = points.map((item) => {
    ...
    return new Marker(item, { icon });
  });

  // 将 marker 加载到图层组
  markerLayerGroup.value = new FeatureGroup(markers);

  // 为每个 marker 绑定 popup
  markerLayerGroup.value.bindPopup('<b>你好啊!</b><br />这是批量创建的 marker');
